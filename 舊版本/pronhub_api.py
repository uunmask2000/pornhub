...
Re_row = 0
# API 服務接口
__api_host = 'http://192.168.8.23:80/'
# -------------------------------
# -----------------------------------------------------
# 爬取網路上資源 或者 轉換 真實ip
...
def new_get_soup(url):
    ...
    # print(_proxy_list_)
    if _proxy_list_ == []:
        print('自己代理有問題等待處理+ 10後重新處理')
        time.sleep(10)
        return new_get_soup(url)
    ##########
    if HTTP == 1:
        ip = random.choice(_proxy_list_)['_url']
        proxies['https'] = ip
    else:
        ip = random.choice(_proxy_list_)['_url']
        proxies['http'] = ip
    # 上鎖
    apiLockProxyStatus(ip)
    # 上鎖
    # print(proxies)
    ...
    try:
        ...
        # 刪除
    # 解鎖
    apiUnlockProxyStatus(ip)
    # 解鎖
    return soup


def apiLockProxyStatus(ip):
    global __api_host
    # http = 0  https = 1
    api_url = str(__api_host) + str('api/apiLockProxyStatus')
    _ip = urlparse(ip).netloc.split(':', 1)[0]
    # print(_ip)
    # 資料
    my_data = {'ip': _ip}
    r = requests.post(api_url, data=my_data)
    print(str('更新') + str(r.content))
    return True


def apiUnlockProxyStatus(ip):
    global __api_host
    # http = 0  https = 1
    api_url = str(__api_host) + str('api/apiUnlockProxyStatus')
    _ip = urlparse(ip).netloc.split(':', 1)[0]
    # print(_ip)
    # 資料
    my_data = {'ip': _ip}
...
def del_proxy(ip):
    global __api_host
    # http = 0  https = 1
    api_url = str(__api_host) + str('api/apiDeleteProxy')
    _ip = urlparse(ip).netloc.split(':', 1)[0]
    # print(_ip)
    # 資料
    my_data = {'ip': ip}
    r = requests.post(api_url, data=my_data)
    print(str('刪除') + str(r.content))
    return True
...
# -------------------驗證方法---------------------------
# --------------------------------------
def get_proxy_list(http=0):
    global __api_host
    global _proxy_list_
    # http = 0  https = 1
    api_url = str(__api_host) + str('api/apiGetProxyOne')
    # 資料
    my_data = {'https': http}
    r = requests.post(api_url, data=my_data)
    #####
