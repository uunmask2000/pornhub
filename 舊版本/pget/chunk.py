...
class Chunk(object):
    ...
    def __init__(self, downloader, url, file, start_byte=-1, end_byte=-1, number=-1,
                 high_speed=False, headers=None, proxies=None):
        self.url = url
    ...
    def resume(self):
        if self.__state == Chunk.PAUSED:
            ...
            self.thread = threading.Thread(target=self.run, kwargs={
                                           'r': self.__paused_request})
            self.thread.start()
        ...
        if r is None:
            if self.start_byte == -1 and self.end_byte == -1:
                ...
                self.headers['Range'] = "bytes=" + \
                    str(self.start_byte) + "-" + str(self.end_byte)
